...
<div class="row"><div style="flex:1"><label>Dashboard Port</label><input id="dashboardPort" value="3360"/></div><div style="flex:1"><label>Live Mode</label><input id="liveMode" value="balanced"/></div></div>
...
<div class="panel"><div class="small" id="dashLabel">Dashboard not started</div><iframe id="dash"></iframe></div>
</main>
<script>
const statusEl=document.getElementById('status'); const logsEl=document.getElementById('logs'); const dash=document.getElementById('dash'); const dashLabel=document.getElementById('dashLabel');
const v=(id)=>document.getElementById(id).value; const c=(id)=>document.getElementById(id).checked;
async function refresh(){ const r=await fetch('/api/state'); const s=await r.json(); statusEl.textContent=s.running?'RUNNING':'IDLE'; const host=location.hostname||'localhost'; const dashUrl=location.protocol+'//'+host+':'+s.dashboardPort; dashLabel.textContent='Dashboard: '+dashUrl; dash.src=dashUrl; }
